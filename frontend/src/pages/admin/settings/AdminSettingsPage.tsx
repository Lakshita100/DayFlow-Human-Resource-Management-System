      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-6 lg:flex-row">
          <div className="w-full shrink-0 lg:w-56">
            <nav className="flex gap-1 rounded-xl border border-gray-100 bg-white p-1.5 shadow-card lg:flex-col">
              {tabs.map((tab) => {
                const Icon = tab.icon;
                return (
                  <button
          ...
          <div className="flex-1">
            {activeTab === 'profile' && (
              <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-card">
                <h3 className="mb-6 text-sm font-semibold text-gray-900">Profile Information</h3>
                <div className="flex items-center gap-5 mb-6">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-400">{user?.role} &middot; {user?.employeeId}</p>
                  </div>
                </div>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <input type="email" defaultValue={user?.email ?? ''} disabled className="w-full rounded-lg border border-gray-200 bg-gray-100 px-3 py-2.5 text-sm text-gray-500" />
                  </div>
                  <div>
                    <label className="mb-1 block text-xs font-medium text-gray-500">Employee ID</label>
                    <input type="text" defaultValue={user?.employeeId ?? ''} disabled className="w-full rounded-lg border border-gray-200 bg-gray-100 px-3 py-2.5 text-sm text-gray-500" />
                  </div>
                  <div>
                    <label className="mb-1 block text-xs font-medium text-gray-500">Role</label>
                    <input type="text" defaultValue={user?.role ?? ''} disabled className="w-full rounded-lg border border-gray-200 bg-gray-100 px-3 py-2.5 text-sm text-gray-500" />
                  </div>
                </div>
                <div className="mt-6 flex justify-end">
                  <button onClick={handleSave} className="inline-flex items-center gap-2 rounded-lg bg-brand-600 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-brand-700">
                    <Save size={16} />
              ...
              <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-card">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={pref.label} className="flex items-center justify-between rounded-lg border border-gray-100 p-4">
                      <div>
            ...
            {activeTab === 'appearance' && (
              <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-card">
                <h3 className="mb-4 text-sm font-semibold text-gray-900">Appearance Settings</h3>
                <div className="space-y-6">
                  <div>
